...
export const predictTemperature = (
  ...
  const sma = simpleMovingAverage(temps, Math.min(5, temps.length))
  ...
  const noiseStd = stdDev(temps) * 0.5 || 0.5
  const zScore = 1.96
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    if (useMethod === 'linear') {
      const futureX = xIndices.length + i
      predValue = lr.slope * futureX + lr.intercept
    } else {
      const decay = Math.exp(-i / (steps * 2))
      predValue = lastSMA + (lr.slope * i) * decay
    }

    const ci = zScore * noiseStd * Math.sqrt(1 + 1 / temps.length)
